...
const DICT = {
  ...
  upfrontBreakdownTitle: { en: "Upfront cost breakdown", ja: "初期費用の内訳" },
  upfrontBreakdownIntro: {
    en: "Paid once, at move-in. Adds up to the upfront total above.",
    ja: "入居時に一度だけ支払う費用です。上記の初期費用合計と一致します。",
  },
  monthlyBreakdownTitle: { en: "Effective monthly breakdown", ja: "実質月額の内訳" },
  monthlyBreakdownIntro: {
    en: "Rent and fees, plus the move-in costs above spread evenly across a standard 24-month lease. Adds up to the effective monthly total above.",
    ja: "家賃・管理費に加え、上記の初期費用を標準的な24ヶ月契約で均等に按分したものです。上記の実質月額と一致します。",
  },
  renewalTitle: { en: "After you move in", ja: "入居後に発生する費用" },
  renewalIntro: {
    en: "Not included in either total above — real costs, but on their own schedule, not every month.",
    ja: "上記のいずれの合計にも含まれていません。実際に発生する費用ですが、毎月ではなく別のタイミングで発生します。",
  },
  subtotal: { en: "Subtotal", ja: "小計" },
  costNotStatedTitle: { en: "Not stated by this listing", ja: "このリスティングに未記載" },
  costDisclaimer: {
    en: "Every amount above is only what this listing actually states — nothing is estimated. Fees below aren't included in any total, so the real cost could be higher.",
    ja: "上記の金額はすべてこのリスティングに実際に記載されているものです（推定なし）。以下の費用はいずれの合計にも含まれていないため、実際の費用はさらに高くなる可能性があります。",
  },
  eligibility: { en: "Eligibility", ja: "入居条件" },
  strategyAdvisorTitle: { en: "Strategy advisor", ja: "交渉戦略" },
  strategyDisclaimer: {
    en: "Not guaranteed — a starting point for a conversation, not an entitlement.",
    ja: "結果を保証するものではありません。会話の出発点としてご活用ください。",
  },
  viewOriginalListing: { en: "View original listing", ja: "元の掲載を見る" },
  noConditionsFound: {
    en: "No specific conditions could be grounded in the listing's text.",
    ja: "掲載文からは具体的な条件を確認できませんでした。",
  },
  askFailed: { en: "Sorry, that request failed. Please try again.", ja: "リクエストに失敗しました。もう一度お試しください。" },
